...
export function NotebookView() {
  ...
  return (
    <div className='max-w-4xl mx-auto py-6 px-4'>
      <h1 className='text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6'>{activeNotebook.name}</h1>
      <div className='space-y-4'>
        {activeNotebook.blocks.map((block, index) => (
          <BlockContainer key={block.id} block={block} notebookId={activeNotebook.id} index={index} />
        ))}
      </div>
  ...
  return (
    <div className='h-full flex items-center justify-center'>
      <div className='text-center'>
        ...
        <h2 className='text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2'>No notebook selected</h2>
